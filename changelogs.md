...
- **Clone Bulk** — right-clicking a pal has a new "Clone Bulk" option that asks how many copies you want, then makes that many of every pal you have selected. Select ten pals, ask for three, and you get three copies of each. It fills the free slots in whatever storage the pals came from, caps the number you can ask for at the space actually available, and tells you how many it made if the slots run out. Works in the palbox, party, DPS storage and Global Pal Storage.
